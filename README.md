...
**[占位: 待一次真实模型调用后填入。]** 生成路径是
`POST /incidents/2/report` → 任务走 `collecting → drafting → validating` 停在
`awaiting_review` → `GET /reports/{id}` 取**渲染后的正文** (五个字段:
summary / handling / impact / notable / suggestion)。填入时一并补上:

- 两个倾向计数 `bare_fact_attempts` / `dangling_ref_attempts` 的实际值 ——
  它们是"模型想在正文里裸写事实 / 引用不存在的 id、**被拦下**"的次数, 按违规项
  累加在 `incident_reports` 的两列里。**为 0 不能说明模型老实, 只能说明这一跑里
  它没试**; 大于 0 恰恰是这套设计在干活的证据;
- 这一次生成的实际花费 (从 `ai_usage` 查: 模型名 / prompt_version / 调用次数 /
  tokens / `estimated_cost_cny` 合计) —— 报数字带产生它的配置, 对花费同样生效。

### 这个对照证明什么

模板那份什么都说了 —— 21 行, 又长又平: "无此记录"与真正要紧的那几条占同样的
版面, 读的人得自己找重点。Agent 那份短, 说的是该说的那几条 (哪条缺失值得点名、
哪个时长值得写进影响)。两份吃的是**同一份事实包**, 差别全部在取舍 ——
**取舍是模型干的活, 事实是代码给的。**
