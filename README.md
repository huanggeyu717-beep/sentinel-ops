...
覆盖率与冗余率这两个数 —— "模型的价值在取舍不在产生事实"这句话是靠这个例子
撑着的, 不要把它读成一个结论。

**对照用的事故** (`basic_spill` 场景包按原速回放产生, 未做任何手工修饰):
Zone 2 单传感器转湿开单, **没有人派单**, 员工 Bo Wang 直接刷卡接单, 传感器转干后
由操作员人工点解决。挑它是因为: 事实包里恰好有三条缺失 (未派单 → "派单给"与
"是否跨区派单"无此记录; 人工解决 → "解决策略"无此记录), 对照里能看见
**"缺失也走占位符"** 这个机制; 时间线 4 条, 不触发截断。

### 模板那份 (脚本产出, 确定性, 零模型调用)

`scripts/dev/render_template_report.py` 吃与 Agent **同一份事实包**
(`report_task_service.load_fact_pack`), 把每条事实按事实包顺序铺成一句话,
不做取舍; 同一条事故跑两次输出逐字节相同 (有测试钉住)。实际输出 (21 行, 全量):
...
传感器: 4 号传感器。
严重级别: 一般。
开单时刻: 2026-08-24 20:24。
接单时刻: 2026-08-24 20:25。
解决时刻: 2026-08-24 20:26。
响应时长 (开单到接单): 27 秒。
处理时长 (开单到解决): 1 分 23 秒。
到场时长 (接单到解决): 56 秒。
派单给: 无此记录。
...
该传感器近三十天开单数: 2 次。
同区同期并发未结事故数: 0 条。
时间线第 1 条: 2026-08-24 20:24 开单。
时间线第 2 条: 2026-08-24 20:25 接单。
时间线第 3 条: 2026-08-24 20:26 传感器转干。
时间线第 4 条: 2026-08-24 20:26 解决。
```

### Agent 那份 (走正常生成路径: 建任务 → 状态机 → 等人过目)

`POST /incidents/2/report` 建任务, 状态机实际走了
`collecting → drafting → validating → repairing → validating`, 停在
`awaiting_review` (那次 `repairing` 见下面第一条); 以下是
`GET /incidents/2/report` 取回的**渲染后正文**, 五个字段, 未做任何人工润色:

```
summary:    Zone 2 - 卖场中区 的 4 号传感器 触发水浸告警, 事故编号 #2, 严重级别 一般。
handling:   系统自动开单后, 由 Bo Wang 接单并到场处置, 最终以 人工 方式确认传感器转干
            并关单; 派单对象记录为 无此记录, 是否跨区派单为 无此记录, 解决策略为 无此记录。
impact:     从开单到接单用时 27 秒, 到场用时 56 秒, 全程处理用时 1 分 23 秒;
            同区同期并发未结事故 0 条, 未造成持续影响。
notable:    本次响应与处置速度极快, 传感器在接单后极短时间内即转干; 该传感器近 2 次
            周期内开单数暂未呈现明显频发趋势。
suggestion:
```

原文照贴, 两处照实说。notable 里"该传感器近 2 次 周期内开单数"读不通:
初稿这句本来通顺 ("该传感器近三十天开单 {{sensor_30d_count}}"), 但"三十天"
作为裸写的中文数字被拦 (即下面计数里的那一次), 修复稿把占位符挪进了"三十天"
原来的句位 —— 语法全对、校验全绿、句子不是人话。

**这一处不是"模型随手放错了地方", 它有确定的成因, 而且是设计留的一个洞**:
那条事实的 label 是"该传感器近三十天开单数", 时间窗是这个指标定义的一部分,
而**事实包里没有任何 id 的 text 是"近三十天"** —— 模型想引用都引用不到,
照着 label 写又会被判据 2 拦下 ("三十天"命中中文数量词, "近 30 天"命中
阿拉伯数字)。**它被逼到没有正确选项。** 两次真实生成都在这一句崩掉,
第一次放弃了语境 ("该传感器 2 次"), 第二次把占位符塞进原来的句位 —— 同一个
死胡同的两种绕法, 不是巧合。修法很小 (给时间窗单独一条事实, 让它能写
`{{sensor_30d_window}}开单 {{sensor_30d_count}} 次`), 记在待办里, v1 不改。

它与文末已知边界那条"用了对的占位符但放错句子, 拦不住"**形状相同、成因不同**:
那一条说的是模型自由选择时可能放错、而校验器分辨不出; 这一处是系统没给它
正确选项。**两者都拦不住, 但只有后者可以靠补一条事实修掉。**suggestion 从初稿起就是空字符串,
修复稿也没补, 原样入库 —— 校验器管的是"写了什么", 不管"该写没写"。

- **两个倾向计数的实际值: `bare_fact_attempts = 1`, `dangling_ref_attempts = 0`。**
  这两列记的是"模型想在正文里裸写事实 / 引用不存在的 id、**被拦下**"的次数。
  那一次就是上面 notable 里的"三十天": 校验器报 `E_BARE_FACT`, 任务转入
  `repairing` 修复后通过。**为 0 的那项不能说明模型老实, 只能说明这一跑里
  它没试**; 为 1 的那次恰恰是这套设计在干活的证据。
- **这一次生成的实际花费** (`ai_usage`, task_id = 12): 模型
  `doubao-seed-2-1-pro-260628`, prompt_version `r1`, 2 次调用 (初稿 + 修复),
  输入 4044 + 输出 484 tokens, `estimated_cost_cny` 合计 **¥0.039** ——
  库与时区配置与模板小节相同。

### 这个对照证明什么

模板那份什么都说了 —— 21 行, 又长又平: "无此记录"与真正要紧的那几条占同样的
版面, 读的人得自己找重点。Agent 那份四句话 (第五句交了白卷): 三条"无此记录"
被收进 handling 一句里, 三个时长进了 impact 且加得平 (27 秒 + 56 秒 =
1 分 23 秒), 近三十天开单数被它拿去支撑"暂未呈现明显频发趋势"的判断 ——
每条事实放进哪句、配什么话, 是它的取舍。它还写了几个事实包里没有的定性说法
("速度极快"、"未造成持续影响"), 这类词占位符机制管不着 —— 文末"已知边界"
第一条说的就是它。两份吃的是**同一份事实包**, 差别全部在取舍 ——
**取舍是模型干的活, 事实是代码给的。**

关于"模型会不会编", 本项目只说两件事, 不折算成任何比率: 模型在正文里**写不出**
自由数字与专名 (语法层面拦住, 是机制不是叮嘱), 而它**想写的次数被记了下来**
(上面两个计数)。这套机制真正的边界也要一并说: **用了对的占位符但放错句子,
...
- **定性形容词拦不住。** "很快就到场了"里没有数字, 占位符机制对它无效。
  这套设计管的是**可核对的事实**, 管不了语气与判断。
- **中文写的计数拦不住。** 中文半边只拦时间与时长, "开了两次单"、"到场两人"、
  "二十余条记录"过得去。丢得起的理由是计数类事实的 `text` 本身就带阿拉伯数字
  ("2 次" / "0 条" / "3 号传感器"), 模型抄它只能抄出阿拉伯数字, 已被阿拉伯数字
  0 容忍那半兜住; 残留的洞只有一种 —— 模型主动把"2 次"译成"两次"。同理"三分"、
  "一个多小时"、"好几个小时"这类不带标准单位的写法也漏。
- **序数一律放行。** "第三次报警"里的"三"拦不住 —— 排除"第X"是为了放行
  "第一时间"与"第一次", 两者形状相同, 按个例修一个漏一个, 所以按规则整体放行。
- **`severity` 与 `resolved_kind` 这两条事实的裸写拦不住。** 它们的中文取值形如
  "一般 / 高 / 紧急"与"人工 / 自动", 都是日常词汇, 进裸写黑名单会把正常句子全打掉。
  这是权衡不是遗漏: 两者各只有三五个固定取值, 编造空间接近零 —— 模型把"高"写成
  "紧急"是分类写错, 不是凭空造事实。
