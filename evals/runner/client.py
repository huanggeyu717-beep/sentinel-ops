...
#
# **不要把这两句改成一个具体默认值** (SPEC-007 补入 39, 已定为不修)。
# 它们"没有给出一个值", 模型于是原样再问一遍直到耗尽三轮 —— L2 那 11 条耗尽的
# 用例里 10 条是这个形状。看上去像个 bug, 但改它有三个问题:
# (a) 那是在看过跑分之后再改一次评测设施并重跑 (约 ¥11), 是拟合;
# (b) 它会抬高分数, 而抬起来的部分不对应任何真实能力提升;
# (c) **更要紧的是它可能不该改** —— 一个只肯回答"你看着办"的用户, 系统的正确行为
#     是什么? 这是**产品判断, 不是评测参数**。现在的答案 ("问到耗尽然后失败")
#     已经作为已知边界写进 README, 当作缺口交付, 而不是当作评测缺陷修掉。
