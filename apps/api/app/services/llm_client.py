...
class LLMResponse:
    ...
    # 按 config 单价估的成本。价目会变, 只用于消融实验的相对比较, 不是财务口径;
    # 回放命中时为 0 (没花钱), 打桩为 0。
...
def cassette_key(
    ...
    清单里, 单独进键只会让"同内容不同标签"失配)。thinking 进键的理由与模型名
    一样: 开关一翻输出就不同, 老录制冒充新配置的行为比 miss 更糟。
    """
...
class RecordReplayLLMClient:
    ...
    async def complete(self, request: LLMRequest) -> LLMResponse:
        key = cassette_key(self.model, self.prompt_version, request, self.thinking)
        path = self._path(key)
        if self.mode == "replay":
            if not path.exists():
            ...
            return LLMResponse(
                **recorded, tool_call=tool_call,
                cache_hit=True, latency_ms=0, estimated_cost_usd=0.0,
            )
